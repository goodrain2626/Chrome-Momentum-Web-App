...
function onLoginSubmit(event){ //form에 이름 다 적고 엔터하면 실행할건 이거야
    event.preventDefault(event); //새로고침 막아줘
    loginBox.style = 'opacity: 0;'
    mainBox.classList.remove(HIDDEN_CLASSNAME); //mainBox 보여줘
...
if(savedUsername === null){ //사용자 이름 저장된 변수에 아무것도 없으면
    //show the form
    loginBox.classList.remove(HIDDEN_CLASSNAME); //form 보여줘
    mainBox.classList.add(HIDDEN_CLASSNAME); //mainBox 가려줘
    loginForm.addEventListener("submit", onLoginSubmit); //submit시키면 onLoginSubmit실행시켜줘
}else{ //사용자 이름 저장된 변수에 이름이 있다면
    //show the greetings
    loginBox.classList.add(HIDDEN_CLASSNAME); //loginBox안보이게
    mainBox.classList.remove(HIDDEN_CLASSNAME); //mainBox 바로보이게
